...
module.exports = {
	...
	getAgentProfile: async (id, result) => {
		try {
	...
	getMainInfo: async (params, result) => {
		let agentId = params;
		let rawQuery = 
		`SELECT a_image1, a_image2, a_image3, a_introduction FROM agent
	...
	updateMainInfo: async (params, body, result) => {
		...
		let res = await db.query(rawQuery, [body.image1, body.image2,body.image3,body.a_introduction, params.agentList_ra_regno]);
		result(res);
	},
	...
	updateEnterdAgentInfo: async (params, body, result) => {
		...
		let res = await db.query(rawQuery, [body.a_profile_image,body.contact_number, body.telno, params.agentList_ra_regno]);
		result(res);
	},
